...
from Formularios import NuevaInterfaz
...
class VgestionFichas(QtGui.QDialog):
    ...
    def __init__(self, parent = None):
        ...
        self.ui = NuevaInterfaz.Ui_Dialog()
        ...
        "Modificar Ficha"
        QtCore.QObject.connect(self.ui.rbModificarEtiqueta, QtCore.SIGNAL('clicked()'),
                               self.rbModificarEtiqueta_click)
        QtCore.QObject.connect(self.ui.rbModificarPorPedido, QtCore.SIGNAL('clicked()'),
                               self.rbModificarPorPedido_click)
        QtCore.QObject.connect(self.ui.cmbModificar, QtCore.SIGNAL('currentIndexChanged (int)'),
                               self.cmbModificar_currentIndexChanged)
        QtCore.QObject.connect(self.ui.pbModificarFicha, QtCore.SIGNAL("clicked()"),
                               self.pbModificarFicha_click)
    ...
    def tabOpFichas_currentChanged(self):
        if self.ui.tabOpFichas.currentIndex() == 0:
            ...
            self.ui.tabModCarSec.setCurrentIndex(0)
            self.ui.tabModificarFicha.setCurrentIndex(0)
            self.estadoceroconsulta()
            self.limpiarFFichaTecnica()
    ...
    def estadoceromodficar(self):
        self.ui.pbModificarFicha.setEnabled(False)
        self.ui.cmbModificar.setCurrentIndex(0)
        self.ui.rbModificarEtiqueta.setAutoExclusive(False)
        self.ui.rbModificarPorPedido.setAutoExclusive(False)
        self.ui.rbModificarEtiqueta.setChecked(False)
        self.ui.rbModificarPorPedido.setChecked(False)
        self.ui.rbModificarEtiqueta.setAutoExclusive(True)
        self.ui.rbModificarPorPedido.setAutoExclusive(True)
        self.ui.cmbModificar.clear()
        self.ui.cmbModificar.addItem("Seleccione Filtro")
